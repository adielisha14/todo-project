...
function UsersDetails() {  
  ...
  <div className='userConteiner flex flex-col items-center justify-center rounded-md w-full mb-8'>
      ...
      <div className='list'>
        ...
        <ul onClick={()=>console.log(use._id)} className='userList rounded-md bg-accent/20 px-5 py-8 my-5 text-2xl font-bold text-primary' key={inx}>
          <div className='flex items-center justify-between'>
            {use.username}
            <TrashIcon onClick={()=>{userDelete(use._id)}} className='delete text-red-600 h-6 w-6'/> 
          </div>
          <img src={use.image?use.image:`https://api.dicebear.com/9.x/bottts-neutral/svg?seed=${Math.floor(Math.random()*1000000)}`} className='userImage h-32 w-[80%] my-1 mx-auto rounded-xl object-cover'></img>
